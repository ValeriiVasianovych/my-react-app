import React from 'react'
import { ListGroup, Button } from 'react-bootstrap'

export default function Todolist({ todoList, setTodoList }) {
  return (
    <ListGroup className="mt-3">
      {todoList.map(todo => (
        <ListGroup.Item key={todo.id} className="d-flex justify-content-between align-items-center p-2">
          {todo.title}
          <Button variant="danger" onClick={() => setTodoList(todoList.filter(t => t.id !== todo.id))}>
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-trash-2">
              <path d="M3 6h18" />
              <path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6" />
              <path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2" />
              <line x1="10" x2="10" y1="11" y2="17" />
              <line x1="14" x2="14" y1="11" y2="17" />
            </svg>
          </Button>
        </ListGroup.Item>
      ))}
    </ListGroup>
  )
}
